...
export class Go {
  ...
  add(x: number, y: number, color: number, isAdd?: boolean) {
    ...
    const eating = this.willEat(move)
    const eatenVertexes = new Set()
    if (isAdd) {
  ...
  willSuicide(move: number[]) {
    const strs = move[2] === Color.BLACK ? this.blackStrs : this.whiteStrs
    const vertex = this.getVertex(move[0], move[1])
    // 该落子是否有气
    const hasLiberty = !!this.getLiberty(vertex).length
    // 有多口气就忽略
    if (hasLiberty) {
      return false
    }
  ...
  willEat(move: number[]) {
    // 将被吃掉的多块棋
    const eatenStrs = new Set<Str>()
    // 当前落黑子，则遍历白棋各块棋， 反之亦然
    const strs = move[2] === Color.BLACK ? this.whiteStrs : this.blackStrs
    //搜索对方每条龙是否是最后1口气
    strs.forEach((str) => {
      // 最后1口气，且坐标位置与落子相同
  ...
  doPlay(move: number[], eating: Set<Str>) {
    const vertex = this.getVertex(move[0], move[1])
    // eat
    const myStrs = move[2] === Color.BLACK ? this.blackStrs : this.whiteStrs
    const oppositeStrs = move[2] === Color.BLACK ? this.whiteStrs : this.blackStrs
  ...
  play(x: number, y: number, color: number): Set<string> | null {
    ...
    // 是否有提子
    const eating = this.willEat(move)
    const eatenVertexes = new Set<string>()
  ...
  getNeighborByColor(x: number, y: number, color: number) {
    ...
    for (const i in neighbors) {
      const x2 = neighbors[i][0] + x
      const y2 = neighbors[i][1] + y
  ...
  getArounds(move: number[]) {
    ...
    for (const i in surround) {
      const x = surround[i][0] + move[0]
      const y = surround[i][1] + move[1]
  ...
  getSgf(startSgf?: string) {
    ...
    if (startSgf) {
      ...
      const index = startSgf.indexOf(key)
