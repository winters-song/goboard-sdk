...
export default class GoboardAnalysisPlayer extends GoboardPlayer{
	...
	* */
    ...
    if(!this.cb || !this.aiAnalysisData){
		...
		if(!this.isBestMove){
			// 添加“实”
			let [x, y] = this.cb?.go2ph(node.col, node.row) || [0,0]
		...
		let [x, y] = this.cb?.go2ph(col, row) || [0,0]
	...
	enterSuggestMove() {
		...
		for(let i = 0; i < this.suggestMoves.length; i++){
			let {col, row} = this.suggestMoves[i];
			if(col === 19 && row === 19){
				continue;
			}
			let key = col + ',' + row;
