...
export default class Goboard {
	...
	* */
	...
	updateHelperLines (col:number, row:number) {
		...
		this.helperLineMark?.transform("t" + (x - this.options.PIECE_RADIUS) + "," + (y - this.options.PIECE_RADIUS)  + 's0.7,0.7');
	...
	*/
	...
	drawTerritories(arr: number[], skipStone:boolean=false, threshold : number=0.6 ) {
		for(let i = 0 ; i< this.options.boardSize ;i++) {
			for (let j = 0; j < this.options.boardSize; j++) {
				...
				if(skipStone && this.pieces[key] && ((arr[index] > 0 &&  this.pieces[key].attrs.color === 1) || (arr[index] < 0 && this.pieces[key].attrs.color === 2))){
					continue
				}
	...
	clearBranchMarkers() {
		for(let k in this.branchMarkers) {
